...
class GaussianCopula(torch.nn.Module):
    ...
    def __init__(self, dim=2):
        ...
        self.sig = nn.Sigmoid()
        self.off_diagonal_val = nn.Parameter(torch.rand(int(self.dim*(self.dim-1)/2)))
        self.diagonal_val = nn.Parameter(torch.ones(self.dim) + torch.ones(self.dim)*torch.rand(self.dim))
    ...
    def get_R(self):
        ...
        off_diag = torch.zeros(self.dim, self.dim)
        ...
        for i in range(self.dim):
            for j in range(i+1):
                if i == j:
                    ...
                    L[i, j] = self.sig(self.diagonal_val[i]) + torch.tensor([1.0])
                else:
                    L[i, j] = off_diag[i,j]
    ...
    def pdf_param(self, x):
        ...
        res = RDet ** (-0.5) * torch.exp(-0.5 * torch.mm(torch.mm(u,(RInv - I)),u.permute(1,0)))
        # print('res:', res)
        if res.data == 0.0:
            print('RDet:', RDet)
    ...
    def mle(self, X, marginals, hyper_param):
        ...
        def log_likelihood():
            ...
            for i in range(n):
                ...
                lh += torch.log(pdf_val if pdf_val.data !=0.0 else torch.tensor([[1e-5]]))
