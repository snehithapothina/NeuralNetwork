...
def load_state_dict(model, state_dict):
    ...
    for name, param in state_dict.items():
        if name in model_state_dict:
            ...
            print(f"Skipping {name} as it is not in the model state dict")
...
def CoCPC_main(logger, args, train_loader, validation_loader, macro_loader):
    ...
    if args.gpu:
        model = CoCPC(args.timestep, args.batch_size, args.seq_len, args.rep_dim ,args.feat_dim,args.vr_num, args.dataset)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logging-dir', type=str, default='log', help='model save directory')
    ...
    parser.add_argument('--seq_len', type=int, default=20, help='window length to sample from each stock')
    ...
    path = os.path.join(args.logging_dir, 'our_model-v1-model_best.pth')
    print(f"Loading checkpoint from: {path}")
    checkpoint = torch.load(path, map_location=torch.device('cpu'))

    load_state_dict(cpc_model, checkpoint['state_dict'])
    cpc_model.eval()

    clsf_model = StockClassifier(args.rep_dim)
    if args.gpu:
        clsf_model = clsf_model
    ...
    logger.info('***** best validation acc :{:.4f}\t mcc:{:.4f}\n'.format(valid_acc_list[best_index], valid_mcc_list[best_index]))
