...
class CoCPC(nn.Module):
    def __init__(self, timestep, batch_size, seq_len, rep_dim, feat_dim, var_num, dataset):
        ...
        self.g_cop = GaussianCopula(dim=var_num)
        self.gru = nn.GRU(rep_dim, rep_dim//2, num_layers=1, bidirectional=False, batch_first=True)
        self.Wk  = nn.ModuleList([nn.Linear(rep_dim//2+64, rep_dim) for i in range(timestep)]) #256, 512
    ...
    def init_hidden(self, batch_size, use_gpu=True):
        if use_gpu: return torch.zeros(1, batch_size, self.rep_dim//2)
    ...
    def macro_context(self, stock_close_price, macro_loader, time_list, t_index):
        ...
        batch_merged_emb = batch_merged_emb
    ...
    def forward(self, x, macro_loader, adj_close_prices, x_time_list, hidden, label):
        ...
        encode_samples = torch.empty((self.timestep,batch,self.rep_dim)).float() # e.g. size 12*8*512
        y = torch.empty((self.timestep, 1, batch)).long()
        ...
        pred = torch.empty((self.timestep, batch, self.rep_dim)).float() # e.g. 12*32*64 size 12*8*512
